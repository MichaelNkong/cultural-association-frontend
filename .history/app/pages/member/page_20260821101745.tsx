...
function hasAdminRole(claims: Record<string, unknown> | null, storedRole?: string | null) {
  if (!claims && !storedRole) return false;

  const roleValues = [claims?.role, claims?.roles, claims?.authorities, claims?.scope, storedRole]
  ...
  return claims?.is_admin === true || claims?.isAdmin === true ||
    roleValues.some(role => role === "ADMIN" || role === "ROLE_ADMIN");
}
...
export default function MemberPage() {
  ...
  const handleAdminToggle = async (user: User) => {
    if (!isAdmin) return;
    const enabled = !user.admin;
    const action = enabled ? "Give" : "Remove administrator access from";
    if (!window.confirm(`${action} ${user.username}?`)) return;
    setPromotingId(user.id);
    setError(null);
    setNotice(null);
    try {
      await MembersAPI.updateAdminRole(user.id, enabled);
      setUsers(currentUsers => currentUsers.map(item => item.id === user.id ? { ...item, admin: enabled } : item));
      setNotice(enabled ? `${user.username} is now an administrator.` : `${user.username} is no longer an administrator.`);
    } catch (error) {
      setError(axios.isAxiosError(error) ? error.response?.data?.error || "Could not update administrator access." : "Could not update administrator access.");
    } finally {
      setPromotingId(null);
    }
  }
            ...
            <table className="members-table">
              ...
              <tbody>
                {loading ? (
                  ...
                  const promoting = promotingId === user.id;
                  return (
                    <tr key={user.id}>
                      ...
                      <td className="members-action-cell">
                        {isAdmin ? (
                          <div className="members-actions">
                            ...
                            <button
                              type="button"
                              className={`members-admin-button ${user.admin ? "is-admin" : ""}`}
                              disabled={promoting}
                              onClick={() => handleAdminToggle(user)}
                            >
                              {promoting ? "Saving..." : user.admin ? "Disable admin" : "Enable admin"}
                            </button>
                          </div>
