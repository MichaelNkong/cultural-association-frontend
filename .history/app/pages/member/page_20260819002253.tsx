...
export default function MemberPage() {
  ...
  if (loading) {
   ...
   const handleCheckboxChange = (memberId: number) => {
    ...
    console.log("states  ", checkedStates);
    if (checkedStates[memberId] === true) {
      console.log("states  ", checkedStates[memberId]);
    }
  };
  ...
  const handleCheck = async (id: number, checked: boolean) =>  {
    ...
    const updatedUsers = userList.map((user) =>
      user.id === id ? { ...user, is_active: checked ? "1" : "0" } : user
    );
    setUserList(updatedUsers);


      const response = await MembersAPI.activateMembersTest(id, checked ? "1" : "0");
      console.log("Response data:", response);  // Debug log for API response
      if (response?.status === 200) {
                    ...
                    {users.length > 0 ? (
                      ...
                      users.map((user, index) => (
                        <tr key={user.id}>
                          ...
                          <td>{user.email}</td>
                          <td>
                            <input type="checkbox" checked={user.active } onChange={() => handleCheckboxChange(user.id)} />
                          </td>
                        </tr>
                      ))
                    ) : (
