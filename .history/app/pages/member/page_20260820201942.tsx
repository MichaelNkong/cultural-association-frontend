...
export default function MemberPage() {
  ...
  const handleMakeAdmin = async (user: User) => {
    if (!isAdmin || user.admin || !window.confirm(`Give ${user.username} administrator access?`)) return;
    setPromotingId(user.id);
    setError(null);
    setNotice(null);
    try {
      await MembersAPI.makeAdmin(user.id);
      setUsers(currentUsers => currentUsers.map(item => item.id === user.id ? { ...item, admin: true } : item));
      setNotice(`${user.username} is now an administrator.`);
    } catch (error) {
      setError(axios.isAxiosError(error) ? error.response?.data?.error || "Could not grant administrator access." : "Could not grant administrator access.");
    } finally {
      setPromotingId(null);
    }
  }
                        ...
                        {isAdmin ? (
                          <div className="members-actions">
                            ...
                            <button
                              ...
                              disabled={Boolean(user.admin) || promoting}
                              onClick={() => handleMakeAdmin(user)}
                            >
                              {promoting ? "Saving..." : user.admin ? "Admin enabled" : "Enable admin"}
                            </button>
                          </div>
