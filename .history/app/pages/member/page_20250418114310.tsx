...
export default function MemberPage() {
  ...
  async function activateMember(id: number): Promise<void> {
    console.log(`Member activation toggled for user with ID: ${id}`);
    ...
    try {
      const response = await MembersAPI.activateMembers(id, "1");
      console.log("Response data:", response);  // Debug log for API response
      if (response?.status === 200) {
        const result = response.data;
            ...
            <div className="col-md-12">
              <div className="table-wrap">
                <table className="table">
                  ...
                  <tbody>
                    {Error && <p className="mb-4 text-sm text-red-500">{message}</p>}
                    {users.length > 0 ? (
                      ...
                      users.map((user, index) => (
                        <tr key={user.id}>
                          ...
                          <td>{user.email}</td>
                          <td>
                            <input type="checkbox" name="selectRow" onClick={(e) => activateMember(user.id)} />
                          </td>
                        </tr>
                      ))
                    ) : (
