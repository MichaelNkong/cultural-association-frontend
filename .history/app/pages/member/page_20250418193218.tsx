...
export default function MemberPage() {
  ...
  async function handleCheckboxChange( user_id: number): Promise<void> {
    ...
    try {
      ...
      const response = await MembersAPI.activateMembersTest(user_id, isChecked? "1": "0");
      console.log("Response data:", response);  // Debug log for API response
      if (response?.status === 200) {
        const result = response.data;
                    ...
                    {users.length > 0 ? (
                      ...
                      users.map((user, index) => (
                        <tr key={user.id}>
                          ...
                          <td>{user.email}</td>
                          <td>
                            <input type="checkbox"name="selectRow"  checked = {e.target.checked}  onClick={(e) => handleCheckboxChange(user.id)}   onChange={(e) => setIsChecked(e.target.checked) }    />
                          </td>
                        </tr>
                      ))
                    ) : (
