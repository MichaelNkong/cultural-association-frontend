...
export default function MemberPage() {
  ...
  async function handleCheckboxChange(event: React.ChangeEvent<HTMLInputElement>, user_id: number): Promise<void> {
      ...
      try {
        const isChecked = event.target?.checked ?? false; // Safely access event.target.checked
        setIsChecked(isChecked);
        const response = await MembersAPI.activateMembers(user_id, isChecked ? "1" : "0");
        console.log("Response data:", response);  // Debug log for API response
        if (response?.status === 200) {
          const result = response.data;
                    ...
                    {users.length > 0 ? (
                      ...
                      users.map((user, index) => (
                        <tr key={user.id}>
                          ...
                          <td>{user.username}</td>
                          <td>{user.email}</td>
                            <input type="checkbox"  checked={isChecked} name="selectRow"    onChange={(event) => handleCheckboxChange(event, user.id)} />
                            <input type="checkbox"  checked={isChecked} name="selectRow"    onChange={() => handleCheckboxChange(user.id)} />
                          </td>
                        </tr>
                      ))
                    ) : (
