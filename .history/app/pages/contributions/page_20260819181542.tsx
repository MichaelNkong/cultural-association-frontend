...
export default function ContributionsPage() {
  ...
  async function handleBulkSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    const amountDue = Number(bulkForm.amountDue);
    if (!Number.isFinite(amountDue) || amountDue < 0) {
      setError("The contribution amount must be zero or greater.");
      return;
    }
    ...
    if (!window.confirm(`Apply ${formatMoney(amountDue)} to every member for ${bulkForm.period}?`)) return;
