...
import ContributionsAPI, { Contribution, ContributionInput, ContributionPaymentUpdate } from "@/app/lib/api/contributions";
...
export default function ContributionsPage() {
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    if (!form.period.trim()) {
      setError("Enter a contribution period.");
      return;
    }
    ...
    try {
      if (editingId) {
        const paymentUpdate: ContributionPaymentUpdate = {
          amountPaid,
        ...
        {admin && (
          <section className="contributions-editor" aria-labelledby="contribution-editor-title">
            ...
            <form className="contributions-form" onSubmit={handleSubmit}>
              <label>
                Member ID
                <input type="number" min="1" step="1" value={form.memberId} onChange={event => setForm({ ...form, memberId: event.target.value })} required />
              </label>
              <label>
                Period
                <input type="text" placeholder="e.g. August 2026" value={form.period} onChange={event => setForm({ ...form, period: event.target.value })} required />
              </label>
              <label>
                Amount due
                <input type="number" min="0" step="0.01" value={form.amountDue} onChange={event => setForm({ ...form, amountDue: event.target.value })} required />
              </label>
              <label>
                Amount paid
