...
export default function ImageResizer() {
    ...
    const [krankenResponse, setKrankenResponse] = useState<KrakenResponse[]>([]);

    kraken.url(params as KrakenParams, function (err: Error | null, data: KrakenResponse) {
        if (err) {
            console.log('Failed. Error message: %s', err);
        } else {
            setKrankenResponse([data]);
            console.log('Success. Optimized image URL: %s', setUrl(data.kraked_url));
            console.log('Success. Optimized image URL: %s', setKrankenResponse([data]));
        }
    });
                ...
                <div className="container">
                    <div className="row justify-content-center">
                        <div className="col-md-6 text-center mb-5">
                            <h2 className="heading-section">Optimized Image</h2>
                        </div>
                    </div>
                    <div className="row">
