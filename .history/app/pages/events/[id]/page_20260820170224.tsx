...
type Registration = { registrationId: number; userId: number; username: string; email: string };
...
export default function EventDetailPage() {
  ...
  useEffect(() => {
    ...
    if (params.id && admin) {
      ...
      EventsAPI.getEventRegistrations(Number(params.id))
        .then((response) => setRegistrations(response.data))
        .catch((requestError) => {
          console.error("Unable to load registrations:", requestError);
          setRegistrationsError("Registered users could not be loaded.");
        })
  ...
  const handleRegistration = async () => {
    ...
    try {
      ...
      setStatus(responseStatus === 409 ? "You're already registered" : "Registration failed");
    }
  };
  ...
  return (
    <main className="events-page">
      <div className="events-shell">
        ...
        {canManage && <section className="event-registrations" aria-labelledby="registrations-title">
          <div className="events-section-heading"><div><p className="events-kicker">Attendance</p><h2 id="registrations-title">Registered users</h2></div><span className="events-count-label">{registrations.length} registered</span></div>
          {registrationsLoading && <p className="events-loading">Loading registered users...</p>}
          {registrationsError && <p className="events-alert">{registrationsError}</p>}
          {!registrationsLoading && !registrationsError && registrations.length === 0 && <p className="events-empty">No users have registered for this event yet.</p>}
          {!registrationsLoading && registrations.length > 0 && <div className="event-registrations__list">{registrations.map((registration) => <div className="event-registration" key={registration.registrationId}><span className="event-registration__index">{String(registration.userId).padStart(2, "0")}</span><strong>{registration.username}</strong><span>{registration.email}</span></div>)}</div>}
        </section>}
      </div>
    </main>
  );
