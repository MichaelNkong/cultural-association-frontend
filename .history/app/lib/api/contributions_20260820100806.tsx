...
class ContributionsAPI {
  ...
  async updateContribution(contributionId: number | string, updates: UpdateContributionRequest) {
    return axios.patch(`${this.baseUrl}${this.contributionsPath}/${contributionId}`, updates, {
      headers: this.headers()
    });
  }

  async applyPaymentToMember(memberId: number | string, amountPaid: number) {
    return axios.patch(`${this.baseUrl}${this.contributionsPath}/member/${memberId}/payment`, {
      amountPaid
    }, {
      headers: this.headers()
    });
  ...
  async applyToAll(period: string, amountDue: number) {
    return axios.post(`${this.baseUrl}${this.contributionsPath}/apply-all`, {
      period,
      amountDue
    }, {
      headers: this.headers()
