  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch("http://localhost:8080/api/auth/register", {
        ...
        body: JSON.stringify(responseCookiesToRequestCookies.),
      });
      ...
      {message && <p className ="mb-4 text-sm text-red-500">{message}</p>}
          <form onSubmit={handleSubmit}  className="flex-col space-y-4">
              <div className ="flex flex-col space-y-2">
                  <input type="text" className="shadow-sm block w-full py-2 sm:text-sm rounded-md text-gray-800 disabled:bg-gray-200 sm:text-sm border-gray-300 focus:ring-primary-500 focus:border-primary-500 placeholder-gray-400 focus:outline-none" placeholder="username" name="username"     onChange={(e) => setFormData.name()}/>
              </div>
              <div className="flex flex-col space-y-2"> 
                  <input type="password" className="shadow-sm block w-full py-2 sm:text-sm rounded-md text-gray-800 disabled:bg-gray-200 sm:text-sm border-gray-300 focus:ring-primary-500 focus:border-primary-500 placeholder-gray-400 focus:outline-none" placeholder="password" name="password" />
              </div>
              <div className="flex flex-col space-y-2">
