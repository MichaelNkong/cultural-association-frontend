...
import ContributionsAPI, { Contribution, ContributionInput } from "@/app/lib/api/contributions";
...
function contributionRows(data: unknown, aggregateByMember = false): ContributionRow[] {
  const rows = extractRows(data).map(item => {
  ...
  if (!aggregateByMember) return rows;

  const aggregated = new Map<string, ContributionRow>();
  rows.forEach(row => {
    const memberKey = String(row.memberId ?? row.memberUsername ?? "unknown");
    const existing = aggregated.get(memberKey);
    if (!existing) {
      aggregated.set(memberKey, { ...row });
      return;
    }

    existing.amountDue += row.amountDue;
    existing.amountPaid += row.amountPaid;
    existing.debt = Math.max(existing.amountDue - existing.amountPaid, 0);
    existing.contributionCount += 1;
    existing.period = "All periods";
...
export default function ContributionsPage() {
  ...
  async function loadContributions(userIsAdmin: boolean) {
    ...
    try {
      ...
      setRows(contributionRows(response.data, userIsAdmin));
    } catch (loadError) {
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    if (!Number.isFinite(amountDue) || amountDue < 0 || !Number.isFinite(amountPaid) || (editingId ? amountPaid <= 0 : amountPaid < 0)) {
      setError(editingId ? "Payment amount must be greater than zero." : "Due and paid amounts must be zero or greater.");
      return;
    }
    ...
    try {
      if (editingId) {
        await ContributionsAPI.applyPaymentToMember(memberId, amountPaid);
  ...
  async function handleBulkSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    setNotice(null);
    try {
      const response = await ContributionsAPI.applyToAll(bulkForm.period, debtIncrement);
      const appliedCount = Array.isArray(response.data) ? response.data.length : 0;

      setNotice(`Contribution applied to ${appliedCount} members.`);
        ...
        {admin && (
          ...
          <section className="contributions-editor" aria-labelledby="contribution-editor-title">
            ...
            <form className="contributions-form" onSubmit={handleSubmit}>
              ...
              <label>
                Payment amount
                <input type="number" min="0.01" step="0.01" value={form.amountPaid} onChange={event => setForm({ ...form, amountPaid: event.target.value })} required={Boolean(editingId)} disabled={!editingId} />
              </label>
              {!editingId && <label>
                Paid date
